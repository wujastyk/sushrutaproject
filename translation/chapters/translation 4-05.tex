...
repeatedly drain the corrupted blood, little by little, that is
...
Alternatively, one may use  oil that is cooked with a decoction of
...
%\textit{apāmārga}.},
    ...
    \gls{atibalā}, and \gls{tṛṇapañcamūlī}
...
%        (grass).}, 
with the admixture of \gls{kākolī}, etc. 

Alternatively, one may use the \gls{balā}-oil that is cooked as
...
%    (\dev{vibhītaka}).}
In that regard, five remedies prepared with milk are described. For
preparing a poultice, milk should be cooked in ghee, oil, fat, marrow,
and \emph{dugdha} 
%    \footnote{In the \SS, the
%        word for milk is \textit{kṣīra} or \textit{payas} but not
%        \textit{dugdha}. Therefore, the word \textit{dugdha} here can
% mean the
%        sap of plants or something that is extracted.}
separately with each of these powdered grains or pulses---barley,
wheat, sesame, 
\gls{mudga}, or 
\gls{māṣa}---that is mixed with
unctuous pastes of 
\gls{kākolī}, 
\gls{kṣīrakākolī}, 
\gls{jīvaka},
...
\gls{piyāla}, 
sugar, 
\gls{kaśeru}, %\footnote{For
% \emph{kaśerukā}},
\gls{surabhi}, and 
\gls{vacā}. Or, the essence of unctuous fruits
%\footnote{Ḍalhaṇa commented \citep[425]{vulgate} that the
%    unctuous fruits mentioned here are sesame, castor, \gls{atasī},
%    \gls{vibhītaka}, etc.}
can be used as a poultice. 

Alternatively, a \se{veśavāra}{minced meat}
%\footnote{In H, the
%    reading \dev{vaiśavāro} does not make sense. It should have been
%    \dev{veśavāro}, as shown in the vulgate, which is the reading we have
%    accepted here.\\ \textit{Veśavāra} is boneless meat minced, steamed,
...
%    \dev{vaiśavāra} is an earlier form of the word \dev{veśavāra}.}
    prepared from the flesh of a fat \gls{cilicima} 
%\footnote{H has
%        the compound word \dev{nalapīnamatsya}. \dev{nalamīna} is a 
%particular
%        fish known as \textit{cilicima} (\dev{cilicimaḥ}). See
%        \textit{Amarakośa}. Also, if the name is \dev{nalamatsya} then the
%        word \dev{pīna} (fat) within the name is not according to proper
%        Sanskrit. But, it can be allowed because the word \dev{matsya} (fish),
%        instead of being a part of the name, can be considered to mean fish in
%        general and thus the word \dev{pīna} becomes its modifier. Thus,
%        \dev{nalapīnamatsya} can mean "a fat fish that is a \dev{nala}
%        (\textit{cilicima})".\\ Ḍalhaṇa says in his comment
%        \citep[425]{vulgate} that \dev{nalamīna} is a type of \dev{rohita}
%        (\textit{rohita}). Monier Williams says that \textit{rohita} is a kind
%        of fish: Cyprinus Rohitaka. Regarding the \textit{rohita} fish, there
%        is a \textit{subhāṣita}: \dev{agādhajalasañcārī na garvaṃ yāti rohitaḥ
%        | aṅguṣṭhodakamātreṇa śapharī pharpharāyate ||} This indicates that
...
%             says that this verse belongs to the \textit{Nītiratna}. I could not find 
%            this text.} 
can be used instead.\footnote{On \dev{veśavāra} “minced meat” see 
    \volcite{1}[787]{josi-maha}, citing \CS\ \Ca{1.27.269}{169} and 
    \SS\ \Su{4.2.25}{409}. Ḍalhana (\emph{idem}) glossed 
