...
\item [22]
One should treat a \se{yoni}{female genital}  that is rough, cold, rigid and \se{alpasparśa}{lacking in sensation} with \se{kumbhīsveda}{pot-sweat}%
	\footnote{%
...
mixed with sweet drugs in the \se{yoni}{vaginas}. And they also should very gently keep \se{balā}{} oil.
...
\item [25]
A physcian should fill \se{yoni}{vagina} that has a bad smell, or also the one that is slimy, with powders prepared from \se{pañcakaṣāya}{five astringent substances}.%
	\footnote{%
...
And in these conditions, one should employ the decoction made from ingredients such as the \se{rājavṛkṣa}{royal tree} as the cleansing agent.%
	\footnote{%
