...
Maṇḍalins are known.  There are thirteen Rājīmants.\footnote{The 
    phrasing of this śloka is awkward.}\q{six mandalinas?}
...
The physician can recognize the \se{sarpita}{serpented} bite as follows.
There are one, two or more deep \se{pada}{locations}\q{prints?} of the
teeth, with little blood.\footnote{Note that \dev{pada} is being used in the
    same sense as in \SS\ chapter 1.13 when describing the place on the body
    where a leech attaches.} And it makes a swelling.\q{problematic 
    phrase}\footnote{Ungrammatical
        phrase.}  They have a \se{cuñcumālaka}{little line of
            puncture-marks},\footnote{The usual dictionary lexeme is \dev{cañcu} 
            “beak,
            billl,” not \dev{cuñcu} as in the Nepalese witnesses.} making them 
            abnormal. They are
