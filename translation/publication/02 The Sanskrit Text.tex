...
(\dev{tilakalka}), ghee (dev{ghṛta}) and honey (\dev{madhu}) are not rare either,
with four instances each in \SS\ and \AS\ and two instances in \AHS. A combination
of the  paste of sesame seeds (\dev{tilakalka}), ghee (\dev{ghṛta}) and honey
(\dev{madhu}) has also been specifically quoted as a healing recipe.\footnote{See
    \SS\ Sū 11.22ab, \AS\ Sū 38.21 and \AHS\ Sū 30.34.}\q{add refs.}
...
healing of ulcers or surgical wounds.\footnote{E.g., \AS\ Sū 37.30 for
    \dev{kṣatakaṇṭha} and \AS\ Ci 17.22 for healing of the surgical wound in
    \dev{udararoga}.}\q{refs}
