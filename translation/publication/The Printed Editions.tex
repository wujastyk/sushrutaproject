...
\dev{pratanuṃ sūcyā bahalam ārayā} is on the right side of the display. 
This entry means that the editors have chosen to read 
\dev{pratanuṃ}, which the reader must infer is attested by K, whereas A has 
\dev{tataḥ} and N and H \dev{pratanū}. Witness A has added the word \dev{picuvartiṃ} after \dev{tataḥ}, omitted \dev{sūcyā bahalam },\footnote{The omitted words are displayed by hovering the cursor over [OM] adjacent to A in the apparatus.} and reads \dev{praveśayet} instead of \de{ārayā}, which is attested by all of the Nepalese witnesses. 
...
\dev{pratanuṃ sūcyā bahalam ārayā} appears in a pop-up window in which the lemma and
